...
Migrating from `nest-postgres`? See [MIGRATION.md](MIGRATION.md) for a step-by-step guide.
...
TypeScript source code (`lib/`) was restored from compiled `dist/` output using [Claude Code](https://claude.ai/claude-code). The original author dropped the source contribution, so the library was reverse-engineered from the published JavaScript and declaration files. All e2e tests pass against the restored source.
